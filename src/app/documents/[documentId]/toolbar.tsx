...
import {
  ListIcon, ListOrderedIcon, AlignLeftIcon, AlignRightIcon,
  AlignJustifyIcon, AlignCenterIcon, LucideIcon, Undo2Icon,
  Redo2Icon, PrinterIcon, SpellCheckIcon, ImageIcon, UploadIcon,
  SearchIcon, BoldIcon, ItalicIcon, UnderlineIcon,
  MessageSquarePlusIcon, ListTodoIcon, RemoveFormattingIcon,
  ChevronDownIcon, HighlighterIcon, Link2Icon,MinusIcon,PlusIcon,ListCollapseIcon
} from "lucide-react";
  ...
  return (
    <div className="relative">
      <button
        ...
        className="h-7 min-w-7 shrink-0 flex items-center justify-center rounded-sm hover:bg-neutral-200/80 px-1.5 text-sm"
      >
      ...
      {open && (
        <>
          ...
          />
          <div
            className="absolute top-full left-0 mt-1 bg-white border border-neutral-200 rounded-md shadow-md p-1 flex flex-col gap-y-1"
            style={{ zIndex: 9999 }}
          >
  ...
  return (
    <DropdownMenu onOpenChange={(open) => {
      ...
      <DropdownMenuContent className="p-2.5 flex items-center gap-x-2">
        <Input
          className="bg-white border-black/20 shadow-none focus-visible:ring-0 focus-visible:ring-offset-0"
          placeholder="https://example.com"
          value={value}
          onChange={(e) => setValue(e.target.value)}
        />
        <Button onClick={() => onChange(value)} className="h-10 bg-black text-white hover:bg-black/90">
          Apply
        </Button>
      </DropdownMenuContent>
    </DropdownMenu>
...
export const Toolbar = () => {
  ...
  const sections: {
    ...
    [
      { label: "Comment", icon: MessageSquarePlusIcon, onClick: () => editor?.chain().focus().addPendingComment().run(), isActive: editor?.isActive("liveblocksCommentMark") },
      { label: "List Todo", icon: ListTodoIcon, onClick: () => editor?.chain().focus().toggleTaskList().run(), isActive: editor?.isActive("taskList") },
  ...
  return (
    <div 
      ...
      style={{ overflow: "visible" }}  // ✅ Yeh add karo
    >
